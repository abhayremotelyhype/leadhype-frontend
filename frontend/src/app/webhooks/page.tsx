...
export default function WebhooksPage() {
  ...
  const fetchWebhooks = async () => {
    try {
      setLoading(true)
      const response = await apiClient.get<any>(ENDPOINTS.webhooks)
      // Handle Node.js backend response format: { success: true, data: [...] }
      const webhooksData = response.data || response
      setWebhooks(Array.isArray(webhooksData) ? webhooksData : [])

      // Fetch events for each webhook
      await fetchWebhookEvents(Array.isArray(webhooksData) ? webhooksData : [])
    } catch (error) {
  ...
  const fetchWebhookEvents = async (webhookList: Webhook[] = webhooks) => {
    try {
      const response = await apiClient.get<any>(ENDPOINTS.webhookEvents)
      // Handle Node.js backend response format: { success: true, data: [...] }
      ...
      webhookList.forEach(webhook => {
        eventsByWebhook[webhook.id] = eventsArray.filter(event => event.webhookId === webhook.id)
      })
