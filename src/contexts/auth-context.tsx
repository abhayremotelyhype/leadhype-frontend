...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshTokenIfNeeded = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle new Node.js backend response format
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (response.ok) {
        const responseData = await response.json();
        // Handle new Node.js backend response format: { success: true, data: { accessToken, refreshToken, user } }
        const data = responseData.data || responseData;
        localStorage.setItem('accessToken', data.accessToken);
        localStorage.setItem('refreshToken', data.refreshToken);
        setUser(data.user);
        ...
        try {
          ...
          return { success: false, error: errorData.error || errorData.message || 'Login failed. Please try again.' };
        } catch (parseError) {
